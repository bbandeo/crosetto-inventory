...
app.post('/ingresos', (req, res) => {
    ...
    console.log(req.session.user);
    if (user != undefined) {
        connection.query(`INSERT into articulo(codbar,descripcion,nombre,operario_ingreso) values('${data.codbar}','${data.marca}','${data.descripcion}','${user.id}')`, function (error, results, fields) {
            if (!error) {
